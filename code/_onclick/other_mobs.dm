...
/mob/living/carbon/human/UnarmedAttack(atom/A, proximity)
	...
	if(!has_active_hand()) //can't attack without a hand.
		to_chat(src, "<span class='notice'>Du schaust deinen Arm an und seufzt.</span>")
		return
...
/atom/proc/can_interact(mob/user)
	...
	if((interaction_flags_atom & INTERACT_ATOM_REQUIRES_DEXTERITY) && !user.IsAdvancedToolUser())
		to_chat(user, "<span class='warning'>Du hast nicht die Fingerfertigkeit, um dies zu tun!</span>")
		return FALSE
...
/mob/living/carbon/monkey/RestrainedClickOn(atom/A)
	...
	if(istype(ML))
		...
		if(prob(75))
			...
			ML.visible_message("<span class='danger'>[name] beist [ML]!</span>", \
							"<span class='userdanger'>[name] beist dich!</span>", null, COMBAT_MESSAGE_RANGE)
			if(armor >= 2)
				return
			...
			ML.visible_message("<span class='danger'>[src]s Biss trift [ML] nicht!</span>", \
							"<span class='danger'>[src]s Biss trift dich nicht!</span>", null, COMBAT_MESSAGE_RANGE)
